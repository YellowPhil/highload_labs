    ...
    for {
        ...
        case input := <- inputChan:
            ...
            if ctx.Err() == context.DeadlineExceeded {
                logger.WarnLogger.Println("Running job takes longer than should. Exitting now...")
                errorCallbackFn(input)
                break
            }
            if err := errorCheckFn(res); err == true {
                logger.WarnLogger.Println("Reader worker received an error in response")
                errorCallbackFn(input)
            } else {
                successCallbackFn(res)
            }
        case <- rw.quitChan:
        logger.InfoLogger.Println("Shutting down reader worker")
        wg.Done()
        return
    }
    }
